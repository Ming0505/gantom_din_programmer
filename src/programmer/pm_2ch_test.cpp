...
static int ch2_test_step = 0;
Button_state button_check(FactoryTest* ft);
class PM_ch2_test : public SmoothSelector
{
    ...
    void onReadInput() override
    {
        ...
        switch(button_check(_ft))
        {
            case Short_pressed:
                ch2_test_step++;
                if(ch2_test_step>3)ch2_test_step=0;
                for(uint8_t i=0;i<8;i++)
                {
                  _pm_dmx->write(i,0);
                }
                switch(ch2_test_step)
                {
                    case 0:
                        _pm_dmx->write(0,255);
                    break;
    ...
    void onRender() override
    {
        ...
        if(ch2_test_step == 0){
            ...
            _ft->_canvas->drawString("CH1: 255",60, 60);
            _ft->_canvas->drawString("CH2:   0",180, 60);
        }else if(ch2_test_step == 1){
            _ft->_canvas->drawCentreString("STEP 2",120, 0);
            _ft->_canvas->drawString("CH1:   0",60, 60);
            _ft->_canvas->drawString("CH2: 255",180, 60);
        }else if(ch2_test_step == 2){
            _ft->_canvas->drawCentreString("STEP 3",120, 0);
            _ft->_canvas->drawString("CH1: 255",60, 60);
            _ft->_canvas->drawString("CH2: 255",180, 60);
        }else if(ch2_test_step == 3){
...
static PM_ch2_test* _launcher_menu = nullptr;

extern uint32_t time_settings;
void pm_2ch_test_task(FactoryTest* ft)
{
    // Create menu
    _launcher_menu = new PM_ch2_test();
