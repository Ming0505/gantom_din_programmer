...
void init_nvs_settings(void)
{
  ...
  { // uninitialized EEPROM
    ...
    preferences.putUInt("DMX_START_CH", 1); //DMX 起始通道值
    preferences.putUInt("DMX_MAX_CH", 127);   //DMX 支持的最大通道数
    preferences.putUInt("DMX_ADDRESS", 0);  //DMX 地址值
  ...
  }else if (preferences.getUChar("initFlag") == 0xA5)
  {
    ...
    DMX_Address_val = preferences.getUInt("DMX_ADDRESS", 1);  //DMX 地址值
...
{
  preferences.putUInt("DMX_START_CH", _start_ch); //DMX 起始通道值
  preferences.putUInt("DMX_MAX_CH", _max_ch);   //DMX 支持的最大通道数
}

void nvs_save_dmxaddress(void)
{
  preferences.putUInt("DMX_ADDRESS", DMX_Address_val);  //DMX 地址值
}
